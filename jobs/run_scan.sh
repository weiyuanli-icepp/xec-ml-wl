...
# Example: Hyperparameter Scanning with Config-based Training
# Usage: ./run_scan.sh
#
# This script demonstrates how to scan hyperparameters using the new
# config-based training system. Modify CONFIG_FILE and override
# parameters via environment variables.

# Base config file (modify paths in this file)
...
# Default values (can be overridden in scan loops)
export EPOCHS=200
export LR="8e-4"
export BATCH=16384
export WEIGHT_DECAY="1e-4"
export EMA_DECAY=""  # Empty = use config default
...
# ===========================================================================
# 4. Scan Weight Decays & ON/OFF EMA
# ===========================================================================
# Test settings (short run)
export EPOCHS=3

# Weight Decays to scan
WEIGHT_DECAYS=("5e-5")
# WEIGHT_DECAYS=("1e-6" "5e-6" "1e-5" "5e-5" "1e-4")

# EMA Decay settings: empty (OFF/use config), "0.999", "0.9999"
EMA_SETTINGS=("")
# EMA_SETTINGS=("" "0.999" "0.9999")

# Learning rates to scan
LEARNING_RATES=("8e-4")
# LEARNING_RATES=("1e-4" "5e-4" "8e-4" "1e-3")

echo "Starting Weight Decay, EMA, Learning Rate scan..."
for WD in "${WEIGHT_DECAYS[@]}"; do
    echo "========================================"
    ...
    for EMA in "${EMA_SETTINGS[@]}"; do
        for LR in "${LEARNING_RATES[@]}"; do
            echo "----------------------------------------"
            echo "Preparing job for WD=$WD | EMA=${EMA:-default} | LR=$LR"
            ...
            # RUN_NAME="scan_wd${WD}_ema${EMA_LABEL}_lr${LR}"
            ...
            echo "Submitting job: WD=$WD | EMA=${EMA:-default} | LR=$LR"
